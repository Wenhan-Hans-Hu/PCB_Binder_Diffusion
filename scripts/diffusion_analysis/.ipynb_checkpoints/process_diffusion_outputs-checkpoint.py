    ...
    def process(q, ref_poses):
        while True:
            ...
            if args.ref is not None:
                ...
                debug1 = [] # LUKAS: This is me trying to figure out how this globbing thing works.
                for r in args.ref:
                    debug1.append(os.path.basename(r).replace(".pdb", "_"))
                debug2 = os.path.basename(pdbfile).split("_")[0]+"_"
                assert len(__refs) == 1, f"Bad number of reference PDBS found for {pdbfile}: {__refs}. Replaced r: {debug1}, found in {debug2}, output is {__refs[0]}"
                ref_pdb = __refs[0]
            else:
