...
if __name__ == "__main__":
    ...
    for N in range(1, NSTRUCT+1):
        ...
        # Using a custom function to find HBond partners of the COO groups. LUKAS: only appropriate for heme!
        # The XML implementation misses some interations it seems.
        for n in range(1, 5):
            df_scores.at[0, f"O{n}_hbond"] = 0.0 #scoring_utils.find_hbonds_to_residue_atom(pose3, ligand_seqpos, f"O{n}")


        # Checking if both COO groups on heme are hbonded. LUKAS: only appropriate for heme!
        if True: #any([df_scores.at[0, x] > 0.0 for x in ['O1_hbond', 'O3_hbond']]) and any([df_scores.at[0, x] > 0.0 for x in ['O2_hbond', 'O4_hbond']]):
        ...
        # LUKAS: I'm hoping that this function isn't actually being used as a score function, it's just being called and the results discarded. Nonetheless, to prevent throwing an error, I've removed references to N4, an atom that doesn't exist in TNT. Eventually I'll have to drop N3 as well for DNT. Oh well, problem for future Lukas, I just really need ProteinMPNN to run. 241126 17:07. Future Lukas is back.
        heme_atoms = []
        target_atom = None # LUKAS: Making sure heme_angle_wrst is never filled in.
