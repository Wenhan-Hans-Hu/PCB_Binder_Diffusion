...
def setup_fastrelax(sfx,ligand_resno, crude=False):
    fastRelax = pyrosetta.rosetta.protocols.relax.FastRelax(sfx, 1)
    if crude is True:
        fastRelax = get_crude_fastrelax(fastRelax)
    fastRelax.constrain_relax_to_start_coords(True)
    ...
    if crude is False:
        ...
        # e.ex1_sample_level(pyrosetta.rosetta.core.pack.task.ExtraRotSample(1))
    tf.push_back(e)
    # Restrict to repacking globally
    tf.push_back(pyrosetta.rosetta.core.pack.task.operation.RestrictToRepacking())
    # Prevent repacking of the ligand
    prevent_repack = pyrosetta.rosetta.core.pack.task.operation.PreventRepackingRLT()
    ligand_res_selector = pyrosetta.rosetta.core.select.residue_selector.ResidueIndexSelector(str(ligand_resno))
    prevent_task = pyrosetta.rosetta.core.pack.task.operation.OperateOnResidueSubset(prevent_repack, ligand_res_selector)
    tf.push_back(prevent_task)
    # Apply TaskFactory to FastRelax
    fastRelax.set_task_factory(tf)
    #Movemap setting
    mm = pyrosetta.rosetta.core.kinematics.MoveMap()
    mm.set_bb(True)
    mm.set_chi(True)
    mm.set_jump(False) #I am shuting it down because mostly its one monomeric protein + one ligand (via jump 1). Please turn it on if you are designing something with cofactor.
    mm.set_bb(ligand_resno, False)
    mm.set_chi(ligand_resno, False)
...
print(f"Ligand_resno: {ligand_resno}")
print(type(ligand_resno))
assert pose.residue(ligand_resno).is_ligand()

fastRelax = setup_fastrelax(sfx, ligand_resno, crude=True)
fastRelax_proper = setup_fastrelax(sfx, ligand_resno, crude=False)
...
###################### Skip ligand rotamer replacement
print(f"Skipping ligand replacement for ligand residue {ligand_resno} to maintain conformational integrity.")

# Retrieve the ligand
ligand = pose.residue(ligand_resno)

# Identify clashes between the ligand and nearby protein sidechains.
clashes = design_utils.find_clashes_between_target_and_sidechains(
    pose, ligand_resno,
    target_atoms=[n for n in range(1, ligand.natoms() + 1) if not ligand.atom_is_hydrogen(n)],
    residues=[n for n in range(1, pose.size() + 1) if n not in catalytic_resnos and n != ligand_resno]
)

# Filter out residues that are already small (ALA, GLY, PRO) or are in keep_native.
clashes = [x for x in clashes if pose.residue(x).name3() not in ["ALA", "GLY", "PRO"] and x not in keep_native]

# If there are any clashing residues, mutate them to ALA and apply FastRelax.
if len(clashes) > 0:
    print(f"Found {len(clashes)} clashing residues near the ligand. Mutating them to ALA and applying FastRelax.")
    pose = design_utils.mutate_residues(pose, clashes, "ALA")
    fastRelax.apply(pose)
else:
    print("No significant clashes found with the ligand.")
...
protocols.relax.RelaxScriptManager: {0} ================== Reading script file: /rds/general/user/wh621/home/miniforge3/envs/diffusion/lib/python3.9/site-packages/pyrosetta/database/sampling/relax_scripts/MonomerRelax2019.txt ==================
...
Skipping ligand replacement for ligand residue 140 to maintain conformational integrity.
Found 8 clashing residues near the ligand. Mutating them to ALA and applying FastRelax.
...
core.pack.dunbrack.RotamerLibrary: {0} Binary rotamer library selected: /rds/general/user/wh621/home/miniforge3/envs/diffusion/lib/python3.9/site-packages/pyrosetta/database/rotamer/beta_nov2016/Dunbrack10.lib.bin
core.pack.dunbrack.RotamerLibrary: {0} Using Dunbrack library binary file '/rds/general/user/wh621/home/miniforge3/envs/diffusion/lib/python3.9/site-packages/pyrosetta/database/rotamer/beta_nov2016/Dunbrack10.lib.bin'.
core.pack.dunbrack.RotamerLibrary: {0} Dunbrack 2010 library took 0.105848 seconds to load from binary
protocols.relax.FastRelax: {0} CMD: repeat  435.243  0  0  0.55
protocols.relax.FastRelax: {0} CMD: coord_cst_weight  435.243  0  0  0.55
protocols.relax.FastRelax: {0} CMD: scale:fa_rep  -71.0884  0  0  0.022
core.pack.task: {0} Packer task: initialize from command line() 
core.pose.selection: {0} [ ERROR ] Could not parse 'ligand_resno' into a residue number.
Segmentation fault (core dumped)
'''

# Re-check for clashes after FastRelax:
post_clashes = design_utils.find_clashes_between_target_and_sidechains(
    pose, ligand_resno,
    target_atoms=[n for n in range(1, ligand.natoms() + 1) if not ligand.atom_is_hydrogen(n)],
    residues=[n for n in range(1, pose.size() + 1) if n not in catalytic_resnos and n != ligand_resno]
)
post_clashes = [x for x in post_clashes if pose.residue(x).name3() not in ["ALA", "GLY", "PRO"] and x not in keep_native]

if len(post_clashes) > 0:
    print("Found no non-clashing rotamers, even after FastRelax.")
    sys.exit(1)
else:
    print("Clashes resolved after FastRelax.")
...
##Wenhan: I have changed {N} to {N:02d}
for N in range(args.nstruct):
    iter_start_time = time.time()
    output_name = f"{pdb_name}_mDE_{N:02d}"
