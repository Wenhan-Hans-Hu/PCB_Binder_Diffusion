...
@author: ikalvet
"""
##WENHAN: Same as section 3.1,but in this file, cst is called in a very organised way, so I didn't touch these. Just fixed the ligand during fastrelax. 
...
####################
    ### Setting up FastRelax with fixed ligand
    ...
    # Set up the TaskFactory with operations that include preventing ligand repacking.
    tf = pyrosetta.rosetta.core.pack.task.TaskFactory()
    tf.push_back(pyrosetta.rosetta.core.pack.task.operation.InitializeFromCommandline())
    tf.push_back(pyrosetta.rosetta.core.pack.task.operation.IncludeCurrent())
    tf.push_back(pyrosetta.rosetta.core.pack.task.operation.NoRepackDisulfides())
    
    # Add extra rotamer sampling
    ...
    # Restrict to repacking overall
    tf.push_back(pyrosetta.rosetta.core.pack.task.operation.RestrictToRepacking())
    
    # Prevent repacking on the ligand residue
    prevent_repack = pyrosetta.rosetta.core.pack.task.operation.PreventRepackingRLT()
    ligand_selector = pyrosetta.rosetta.core.select.residue_selector.ResidueIndexSelector(str(ligand_seqpos))
    prevent_task = pyrosetta.rosetta.core.pack.task.operation.OperateOnResidueSubset(prevent_repack, ligand_selector)
    tf.push_back(prevent_task)
    
    fastRelax.set_task_factory(tf)
    
    # Set up the MoveMap. Here we disable backbone and chi moves for the ligand, and turned off jump to prevent rigid movement.
    movemap = pyrosetta.rosetta.core.kinematics.MoveMap()
    movemap.set_jump(False)
    movemap.set_chi(True)
    movemap.set_bb(True)
    movemap.set_bb(ligand_seqpos, False)  # Fix the ligand backbone
    movemap.set_chi(ligand_seqpos, False)  # Fix the ligand sidechain
    fastRelax.set_movemap(movemap)

    # ### Setting up FastRelax (Old way)
    # fastRelax = pyrosetta.rosetta.protocols.relax.FastRelax(sfx_cst, 1)
    # fastRelax.constrain_relax_to_start_coords(True)
    ...
    for N in range(args.nstruct):
        ...
        # Heme models only:
        # LUKAS: I have some concerns that this is really being applied to "Heme models only", so I've taken the liberty of knocking out this if statement on a more permanent basis. UPDATE: restored during bugfixing, I fear a horrible mistake.
