...
def create_slurm_submit_script(filename, gpu=False, gres=None, time=None, mem="2g", N_nodes=1, N_cores=1, name=None, array=None, array_commandfile=None, group=None, email=None, command=None, outfile_name="output", env="diffusion"):
    
    submit_txt = "#!/bin/bash\n" # LUKAS: turns out you don't have to specify :gpu_type=RTX600, PBS figures that out on its own! Now all our scripts are compatible with all the HPC queues regardless of what GPUs they contain. 
    
    if gpu == True:
        submit_txt += f'''#PBS -l walltime=24:00:00
#PBS -l select=1:ncpus={N_cores}:mem={mem}b:ngpus=1
'''
    else:
        submit_txt += f'''#PBS -l walltime=08:00:00
#PBS -l select=1:ncpus={N_cores}:mem={mem}b
'''
        
    if group is not None:
        numJobs = math.ceil(array/group)
        submit_txt += f"#PBS -J 1-{numJobs}\n"
    elif array is not None:
        submit_txt += f"#PBS -J 1-{array}\n"
        
    submit_txt += \
    f'''module load miniforge/3
module load CUDA/12.6.0
eval "$(~/miniforge3/bin/conda shell.bash hook)"
conda activate /rds/general/user/wh621/home/miniforge3/envs/{env}
cd $PBS_O_WORKDIR
echo 'Starting at:' $(date +%F) $(date +%T)
'''
##WENHAN: module load CUDA/12.6.0 on cx3-phase2 hpc instead of cuda/11.4.2 on on cx3-phase1 hpc. 
    ## Besides, for whoever using this pipeline, change 'wh621' to your college username to work.
    if group is not None:
        submit_txt += \
    f""" 
if (($PBS_ARRAY_INDEX * {group} > {array})); then                                        
    let "FINAL_LINE={array}"
    let "FIRST_LINE=$PBS_ARRAY_INDEX*{group}-{group}"                                      
    let "TAIL=$FINAL_LINE-$FIRST_LINE"
    head -n $FINAL_LINE ./{array_commandfile} | tail -n $TAIL | bash
else                                                                                     
    let "FINAL_LINE=$PBS_ARRAY_INDEX*{group}"                                              
    head -n $FINAL_LINE ./{array_commandfile} | tail -n {group} | bash
fi\n
""" 
##WENHAN: let"" is required in hpc2
    elif array_commandfile is not None:
        submit_txt += f"head -n $PBS_ARRAY_INDEX ./{array_commandfile} | tail -n 1 | bash \n"
    else:
        submit_txt += f"{command}\n"

    submit_txt += "echo 'Finishing at:' $(date +%F) $(date +%T)\n"

    with open(filename, "w") as file:
        for l in submit_txt:
            file.write(l)

#Input JSON file generation for Protenix:
